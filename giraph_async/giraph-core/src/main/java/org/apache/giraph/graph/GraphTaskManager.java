...
/*if_not[PURE_YARN]
  ...
  public void setup(Path[] zkPathList)
    ...
    //Vicky
    this.local_supersteps = new ConcurrentHashMap<Integer,Long>();
  ...
  */
  public void execute() throws IOException, InterruptedException {
    ...
    //Vicky
    //LOG.debug("Vicky ----> execute: Set up execution");
    ...
    //Vicky <----------------------------------------------------------------
    //Vertex assignments should not change during computation
    ...
    for (Integer partitionId : partitionStore.getPartitionIds()) {
      local_supersteps.put(partitionId,0L);
    }

    // main superstep processing loop
    while (!finishedSuperstepStats.allVerticesHalted()) {
      //LOG.debug("Vicky --> execute: In while loop: for every superstep: ");
      ...
      //Vicky
      //LOG.debug("Vicky --> GraphTaskManager.execute.serviceWorker.startSuperstep");
      Collection<? extends PartitionOwner> masterAssignedPartitionOwners =
        serviceWorker.startSuperstep(); // Vicky <-----------------------
      if (LOG.isDebugEnabled()) {
        LOG.debug("execute: " + MemoryUtils.getRuntimeMemoryStats());
      }
      context.progress();
      //LOG.debug("Vicky ---> exchange vertex partitions");
      serviceWorker.exchangeVertexPartitions(masterAssignedPartitionOwners);
      context.progress();
      //LOG.debug("Vicky ---> after exchange of partitions");
      ...
      //Vicky
      MessageStore<I, Writable> extended_incoming_messageStore =
              serviceWorker.getServerData().getIncomingExtendedMessageStore();
      //Vicky 2
      MessageStore<I, Writable> incoming_messageStore =
              serviceWorker.getServerData().getIncomingMessageStore();
      //LOG.info("GraphTaskManager.execute.getCurrentMessageStore = " + messageStore);
      ...
      if (numPartitions > 0) {
        processGraphPartitions(context, partitionStatsList, graphState, //Vicky <---------------------
                messageStore,incoming_messageStore, numPartitions, numThreads);
      }
   ...
   */
  public Map<Integer,Long> getLocalSupersteps()
  {
    return this.local_supersteps;
  }
  ...
  private void  processGraphPartitions(final Mapper<?, ?, ?, ?>.Context context,
      ...
      final MessageStore<I, Writable> incoming_messageStore,
      int numPartitions,
      int numThreads) {
      ...
      new CallableFactory<Collection<PartitionStats>>() {
        ...
        public Callable<Collection<PartitionStats>> newCallable(
          ...
          return new ComputeCallable<I, V, E, Writable, Writable>(
              ...
              graphState,
              messageStore,
              incoming_messageStore,
              computePartitionIdQueue,
              conf,
              serviceWorker,
              local_supersteps, serviceWorker.getSuperstep()); //Vicky: added local and global superstep
        }
      };
