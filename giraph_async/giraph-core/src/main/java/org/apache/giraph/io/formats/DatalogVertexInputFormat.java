                ...
                while(true) {
                    if(getRecordReader().nextKeyValue()) {
                        ...
                        if (tokens[0].equals("edge")) {
                            LOG.info("Read edge to neighbor " + tokens[1]);
                            edges.add(EdgeFactory.create(new LongWritable(Long.parseLong(tokens[1])), new FloatWritable(Float.parseFloat(tokens[2]))));
                        } else {
                            LOG.info("Initialize vertex object");
                            vertex.initialize(id, value, edges);
                            return vertex;
                        }
                    }
                    else{
                        LOG.info("No more lines");
                        break;
                    }
                }
            }
