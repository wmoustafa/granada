...
public class SendWorkerMessagesRequest<I extends WritableComparable,
   ...
   */
  public SendWorkerMessagesRequest(
          PairList<Integer,SenderIdIterationNumber> sender, VertexIdMessages<I, M> v_data) {
    this.pId_sender_info = sender;
    this.vertex_data = v_data;
    addMessageTime = 0;
      addMessageCounter = 0;
  }
  ...
  public void doRequest(ServerData serverData) {
      ...
      try {
        ...
        ((LongByteArrayMessageStore)serverData.getIncomingMessageStore()).
              addExtendedPartitionMessages(this.pId_sender_info,this.vertex_data);
      } catch (IOException e) {
          throw new RuntimeException("doRequest: Got IOException ", e);
