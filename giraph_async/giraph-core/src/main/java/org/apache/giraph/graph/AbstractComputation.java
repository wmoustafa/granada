...
public abstract class AbstractComputation<I extends WritableComparable,
  ...
  /**
   * Vicky
   * Send a message to a vertex id including the sender id.
   *
   * @param id Vertex id to send the message to
   * @param message Message data to send
   */
  public void sendExtendedMessage(Vertex<I,V,E> sender, I id, M2 message) {
    workerClientRequestProcessor.sendExtendedMessageRequest(sender, id, message);
  }
   ...
   */
  public void sendExtendedMessageToAllEdges(Vertex<I, V, E> vertex, M2 message) {
    workerClientRequestProcessor.sendExtendedMessageToAllRequest(vertex, message);
  }
