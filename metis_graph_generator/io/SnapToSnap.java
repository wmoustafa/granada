...
import it.unimi.dsi.fastutil.ints.Int2ByteOpenHashMap;
	...
	public class SnapToSnap {
		...
		public static void main(String[] args) throws Exception
		{
	        ...
	        if (args[3].equals("0")) 
	        	NUM_LEVEL2_PARTITIONS = NUM_LEVEL1_PARTITIONS; 
	        else 
	        	NUM_LEVEL2_PARTITIONS = Integer.parseInt(args[3]);
	        
	        if (args[4].equals("0")) 
	        	NUM_LEVEL3_PARTITIONS = (int)Math.sqrt(NUMBER_OF_NODES/NUM_LEVEL1_PARTITIONS/NUM_LEVEL2_PARTITIONS); 
	        else 
	        	NUM_LEVEL3_PARTITIONS = Integer.parseInt(args[4]);
			...
			for (int i = 0; i < NUM_LEVEL1_PARTITIONS; i++)
			{
				...
				int[][][] level2Graphs = getGraphPartitions(level1Graphs[i], level2VertexPartitions, 
						NUM_LEVEL2_PARTITIONS);
				...
				{
					int[] level3VertexPartitions = getNodePartitions(level2Graphs[j], 
							NUM_LEVEL3_PARTITIONS, tmpDir);
			...
			int[][][][] verticesInEachPartition = getVerticesInEachPartition(graph, threeLevelPartitioning, 
					NUM_LEVEL1_PARTITIONS, NUM_LEVEL2_PARTITIONS, NUM_LEVEL3_PARTITIONS);
			writeHdfsFile(graph, OUTPUT_FILE, CSV_FILE, NUMBER_OF_LABELS, NUMBER_OF_NODES, 
					verticesInEachPartition, threeLevelPartitioning, NUM_LEVEL1_PARTITIONS, 
					NUM_LEVEL2_PARTITIONS, NUM_LEVEL3_PARTITIONS);
		...
		static int[][][][] getVerticesInEachPartition(int[][] graph, int[][] threeLevelPartitioning, 
				int numLevel1Partitioning, int numLevel2Partitioning, int numLevel3Partitioning)
		{
		...
		static void writeHdfsFile(int[][] graph, String outputFile, String csvFile, int numberOfLabels, 
				int numberOfNodes, int[][][][] verticesInEachPartition, int[][] threeLevelPartitioning, 
				int numLevel1Partitioning, int numLevel2Partitioning, int numLevel3Partitioning)
						throws Exception
		{
			...
			BufferedWriter giraphOut = new BufferedWriter(new PrintWriter( outputFile));
			...
			for (int i = 0; i < numLevel1Partitioning; i++)
				for (int j = 0; j < numLevel2Partitioning; j++)
					for (int k = 0; k < numLevel3Partitioning; k++)
					{
						...
						Random r = new Random();
						Set<int[]> outEdges = null;
						for (int n : verticesInEachPartition[i][j][k])
						{
							 outEdges = new HashSet<int[]>();
...
//								}
							...
							int[] vertexData = new int[2];
							vertexData[0] = n;
							vertexData[1] = labelNumber;
						
							// FIXME Vicky: removing in_edges
							Object[] vertexArray = new Object[2]; 
							vertexArray[0] = vertexData;
							vertexArray[1] = outEdges;
...
//						System.out.println(new Gson().toJson(superVertexArray));
						try {
							//FIXME Vicky replace json
							//Print super-vertex data per line
							giraphOut.write("[");						
							//print super-vertex id
							giraphOut.write("["+((int[])superVertexArray[0])[0]+ "," +
									((int[])superVertexArray[0])[0] +"]");
									
							//print vertex data
							giraphOut.write("[");
							for(int ii=0; ii< ((Object[])superVertexArray[1]).length; ii++)
							{	
								int[] vertex = (int[])((Object[])((Object[])superVertexArray[1])[ii])[0];
								giraphOut.write("("+ vertex[0] + ","+ vertex[1] + ")");	

								// print out edges per vertex
								Set<int[]>edges = (Set<int[]>)((Object[])((Object[])superVertexArray[1])[ii])[1];
								giraphOut.write("[");
								for (int[] outEdge : edges)
									giraphOut.write("(" + outEdge[0] + "," + outEdge[1] + ")");
								giraphOut.write("]"); //end of edges
							}
							giraphOut.write("]"); //end of vertices
							
							//print sv neighbor edges
							giraphOut.write("[");
							Set<List<Integer>> sv_neighbors = ((Set<List<Integer>>)superVertexArray[3]);
							for( List<Integer> neighbor: sv_neighbors)
							{
								giraphOut.write("(");
								giraphOut.write(neighbor.get(0) + "," + neighbor.get(1) + "," + neighbor.get(2));
								giraphOut.write(")");							
							}
							giraphOut.write("]"); //end of sv neighbors
...
//							giraphOut.println(new Gson().toJson(superVertexArray));
		...
		static int[][] getGraph(String filename, int numberOfNodes) throws Exception
		{
			...
			boolean[] isVertex = new boolean[numberOfNodes];
			...
			for (int i = 0; i < numberOfNodes; i++)
			{
				...
				graph[i] = new int[numNeighbors_i + 1];
			    }
			}
