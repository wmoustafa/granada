...
/*end[HADOOP_NON_SECURE]*/
    ...
    LOG.debug("Vicky --? BspServiceWorker.setup calls finishSuperstep");
  ...
  public FinishedSuperstepStats finishSuperstep(
    ...
    long addMessageTime=0;
    long getMessageTime=0;
    long computeInvocations = 0;
    long orderVerticesTime = 0;
      long addMessageCounter=0;
      long getMessageCounter=0;
    ...
    for (PartitionStats partitionStats : partitionStatsList) {
      ...
      addMessageTime += partitionStats.getAddMessageTime();
      getMessageTime += partitionStats.getGetMessageTime();
      computeInvocations += partitionStats.getComputeInvocations();
      orderVerticesTime += partitionStats.getOrderVerticesTime();
        addMessageCounter += partitionStats.getAddMessageCounter();
        getMessageCounter+= partitionStats.getGetMessageCounter();
    ...
    if (incomingMessageStore instanceof AsyncMessageStoreWrapper) {
      LOG.debug("Vicky --> AsyncMessageStoreWrapper");
    ...
    if (LOG.isInfoEnabled()) {
      LOG.info("finishSuperstep: Superstep " + getSuperstep() +
          ...
          " , addMessageTime = " + addMessageTime +
          " , getMessageTime = " + getMessageTime +
              " , computeInvocations = " + computeInvocations +
            " , orderVertices = " + orderVerticesTime+
    ...
    writeFinshedSuperstepInfoToZK(partitionStatsList,
            workerSentMessages, workerSentMessageBytes, addMessageTime, getMessageTime,
            computeInvocations, orderVerticesTime, addMessageCounter, getMessageCounter);
  ...
  private void waitForRequestsToFinish() {
    if (LOG.isInfoEnabled()) {
      LOG.info("waitForRequestsToFinish: Waiting on all requests, superstep " +
          getSuperstep() + " " +
          MemoryUtils.getRuntimeMemoryStats());
    }
  ...
  private void writeFinshedSuperstepInfoToZK(
      ...
      long workerSentMessageBytes, long addMessageTime, long getMessageTime,
      long computeInvocations, long orderVerticesTime,long addMessageCounter, long getMessageCounter) {
    ...
    try {
      ...
      workerFinishedInfoObj.put(JSONOBJ_NUM_MESSAGE_BYTES_KEY,
        ...
        workerFinishedInfoObj.put(JSONOBJ_ADD_MESSAGE_TIME, addMessageTime);
        workerFinishedInfoObj.put(JSONOBJ_GET_MESSAGE_TIME, getMessageTime);
        workerFinishedInfoObj.put(JSONOBJ_COMPUTE_INVOCATIONS, computeInvocations);
        workerFinishedInfoObj.put(JSONOBJ_ORDER_VERTICES, orderVerticesTime);
        workerFinishedInfoObj.put(JSONOBJ_ADD_MESSAGE_COUNTER, addMessageCounter);
        workerFinishedInfoObj.put(JSONOBJ_GET_MESSAGE_COUNTER, getMessageCounter);
