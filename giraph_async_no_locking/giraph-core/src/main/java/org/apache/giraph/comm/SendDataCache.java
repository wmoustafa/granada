...
public abstract class SendDataCache<D> {
  ...
  /** Class logger */
  private static final Logger LOG = Logger.getLogger(SendDataCache.class);
  ...
  removeWorkerData(WorkerInfo workerInfo) {
    ...
    for (Integer partitionId : partitions) {
      if (dataCache[partitionId] != null) {
        ...
        /*LOG.debug("Vicky -->  workerData is of type = " + workerData.getClass().getSimpleName());
        PairList.Iterator i = workerData.getIterator();
        while (i.hasNext()) {
          i.next();
          LOG.debug("Vicky -->" + "[partition? " + i.getCurrentFirst().toString() +
                  ", vID, message " + i.getCurrentSecond().toString() + "]");
          if (i.getCurrentSecond() instanceof ByteArrayVertexIdMessages) {
            LOG.debug("Vicky --> bytearray size = " +  ((ByteArrayVertexIdMessages) i.getCurrentSecond()).getByteArray().length);
            if (((ByteArrayVertexIdMessages) i.getCurrentSecond()).getByteArray().length > 0) {
              VertexIdMessageIterator vertexIdMessageBytesIterator =
                      ((ByteArrayVertexIdMessages) i.getCurrentSecond()).getVertexIdMessageIterator();

              while (vertexIdMessageBytesIterator.hasNext()) {
                vertexIdMessageBytesIterator.next();
                LOG.debug("Vicky --> [vertex = " + vertexIdMessageBytesIterator.getCurrentVertexId() + ", message = " +
        ...
        LOG.debug("Vicky --> datacache contains objects of instance " + dataCache[partitionId].getClass().getSimpleName());*/
