...
public class UnsafeArrayReads extends UnsafeReads {
  ...
  private static final Logger LOG =
          Logger.getLogger(UnsafeArrayReads.class);
  ...
  public double readDouble() throws IOException {
    //LOG.debug("Vicky --> readDouble, buf=" + new String(buf,"UTF-8") + ",size=" + buf.length + ", pos = " + pos +
  ...
  //Vicky
  public double readDoubleWithSkip(int from) throws IOException {

    int skipped = skipBytes(from);
    if(skipped < from)
      throw new RuntimeException("Skipping Bytes not working");
            ensureRemaining(SIZE_OF_DOUBLE);
    double value = UNSAFE.getDouble(buf,
            BYTE_ARRAY_OFFSET + pos);
    pos += SIZE_OF_DOUBLE;
    //LOG.info("Vicky --> readDouble value = " + value);
