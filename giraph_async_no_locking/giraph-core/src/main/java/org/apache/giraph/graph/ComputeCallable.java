...
public class ComputeCallable<I extends WritableComparable, V extends Writable,
  ...
  public Collection<PartitionStats> call() {
    ...
    LOG.info("Vicky --> TOTAL Messages sent at superstep : " + global_superstep + " = " + message_counter);
    LOG.info("Vicky ---> Compute invocations at superstep " +
            global_superstep + " = " + compute_incovations_counter);
   ...
   */
  private PartitionStats computePartitionAsync( Computation<I, V, E, M1, M2> computation, Partition<I, V, E> partition)
    ...
    synchronized (partition) {
      ...
      if(global_superstep == 0) {
       ...
       //while (((SimpleMessageStore) incoming_messageStore).get_has_messages_map(partition.getId()).size() > 0) {
         //LOG.info("Vicky --> In while loop superstep = " + global_superstep);
         computePartitionForLoop(computation, partition, partitionStats,verticesComputedProgress);
         //LOG.info("Vicky --> finished one for loop");
       //}
      }
    }
   ...
   * @return Partition stats for this computed partition
   */
  private void computePartitionForLoop(Computation<I, V, E, M1, M2> computation, Partition<I, V, E> partition,
    ...
    for(Object vertex_id: ((SimpleMessageStore) incoming_messageStore).get_has_messages_map(partition.getId()).keySet()) {
      ...
      //TODO in comments when no-while
    //for (Vertex<I, V, E> vertex : partition) {
      ...
      Iterable<M1> messages = ((ByteArrayMessagesPerVertexStore) incoming_messageStore).getVertexMessagesImproved(
              vertex.getId(), partition.getId());
      ...
      if (!vertex.isHalted()) {
        ...
        try {
          ...
          int current_pos = ((ByteArrayMessagesPerVertexStore) incoming_messageStore).getCurrentPos(partition.getId(), vertex.getId());
          DataInputOutput dataInputOutput =
                  (DataInputOutput)((ByteArrayMessagesPerVertexStore) incoming_messageStore).getPartitionMapByPartitionId(partition.getId()).get(((LongWritable) vertex.getId()).get());
          int elements = (((UnsafeByteArrayOutputStream) dataInputOutput.getDataOutput()).getPos() - current_pos);
          LOG.info("Vicky --> vertex = " + vertex.getId() + ", " +
                  "current pos = " + current_pos +
                  ", end pos = " + ((UnsafeByteArrayOutputStream) dataInputOutput.getDataOutput()).getPos() +
                  " , number of elements = " + elements);
        }
          compute_incovations_counter++;
          vertex.voteToHalt();
          ((ByteArrayMessagesPerVertexStore) incoming_messageStore).clearHasNewMessages(partition.getId(),  vertex.getId()); //TODO for while loop
        //------------------------------------------------------------
    ...
    ((SimpleMessageStore) incoming_messageStore).resetAllVertexMessageMap(partition.getId()); //TODO for while loop
  }
