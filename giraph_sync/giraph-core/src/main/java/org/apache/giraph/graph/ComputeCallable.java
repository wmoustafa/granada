...
public class ComputeCallable<I extends WritableComparable, V extends Writable,
  ...
  //Vicky
  private long compute_incovations_counter;
  ...
  public Collection<PartitionStats> call() {
    ...
    LOG.info("Vicky --> TOTAL Messages sent at superstep : " + graphState.getSuperstep() + " = " + message_counter);
    LOG.info("Vicky SYNC ---> Compute invocations  at superstep " +
            graphState.getSuperstep() + " = " + compute_incovations_counter);
  ...
  private PartitionStats computePartition(
    ...
    synchronized (partition) {
      //((LongByteArrayMessageStore)messageStore).printMap();
      for (Vertex<I, V, E> vertex : partition) {
        ...
        if (!vertex.isHalted()) {
          ...
          /*LOG.debug("Vicky SYNC ---> Current vertex=" + vertex.getId() + ":" + vertex.getValue() + ", getVertexMessages at superstep "
                  + graphState.getSuperstep());
          StringBuilder sb = new StringBuilder("Vicky SYNC --> messages = [ ");
          for (M1 message : messages) {
            sb.append(message + ", ");
          }
          sb.append("]");
          LOG.debug("Vicky SYNC --> " + sb.toString());*/
          //------------------------------------------------------------
